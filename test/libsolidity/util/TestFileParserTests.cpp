...
BOOST_AUTO_TEST_CASE(call_effects)
{
	...
	BOOST_REQUIRE_EQUAL(calls.size(), 1);
	BOOST_REQUIRE_EQUAL(calls[0].sideEffects.size(), 3);
	BOOST_REQUIRE_EQUAL(boost::trim_copy(calls[0].sideEffects[0]), "bla");
	BOOST_REQUIRE_EQUAL(boost::trim_copy(calls[0].sideEffects[1]), "bla bla");
	BOOST_REQUIRE_EQUAL(boost::trim_copy(calls[0].sideEffects[2]), "bla bla bla");
	source = R"(
		// builtin_returning_call_effect -> 1
		// - bla
		// - bla bla
	...
	BOOST_REQUIRE_EQUAL(calls.size(), 3);
	BOOST_REQUIRE_EQUAL(calls[0].sideEffects.size(), 2);
	BOOST_REQUIRE_EQUAL(calls[1].sideEffects.size(), 1);
	BOOST_REQUIRE_EQUAL(calls[2].sideEffects.size(), 0);
	BOOST_REQUIRE_EQUAL(boost::trim_copy(calls[0].sideEffects[0]), "bla");
	BOOST_REQUIRE_EQUAL(boost::trim_copy(calls[0].sideEffects[1]), "bla bla");
	BOOST_REQUIRE_EQUAL(boost::trim_copy(calls[1].sideEffects[0]), "bla bla bla");
}
